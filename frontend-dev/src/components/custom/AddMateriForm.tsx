// src/components/custom/AddMateriForm.tsx
...
const AddMateriForm: React.FC<AddMateriFormProps> = ({ onAddMateri, onCancel, initialData }) => {
  ...
  const apiUrl = import.meta.env.VITE_API_URL || ""; // gunakan env untuk base API
  ...
  const uploadImageToServer = async (file: File): Promise<string> => {
    ...
    if (!res.ok) {
      ...
      throw new Error(txt || "Upload gambar gagal");
    }
  ...
  // IMAGE HANDLER: upload to server, then insert returned URL into Quill
  const imageHandler = () => {
    ...
    input.onchange = async () => {
      ...
      try {
        // Optional: add to articleImages state if you want to track uploaded files locally
                ...
                {!jenisMateri && (
                  <div className="flex flex-col items-center justify-center h-full min-h-[300px] border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50 text-gray-400 p-6 text-center">
                    ...
                    <p className="text-sm font-medium">
                      Pilih <strong>Jenis Materi</strong> terlebih dahulu!
                    </p>
                  </div>
                )}
                        ...
                        <div className="flex flex-col gap-4 h-full">
                          <div className="bg-red-50 border border-red-100 p-6 rounded-xl flex flex-col items-center justify-center text-center min-h-[200px]">
                            ...
                            <p className="text-xs text-red-600">
                              Video akan disematkan otomatis
                            </p>
                          </div>
                          <FormControl>
                            <Input
                    ...
                    render={({ field, fieldState: { error } }) => (
                      <FormItem className="flex flex-col">
                        <FormLabel className="mb-2">
                          Isi Artikel / Materi Pembelajaran
                        </FormLabel>
