...
import AddMateriDialog from "./AddMateriToTopic";

// --- API CONFIGURATION & HELPER (Moved from topicService.ts) ---
...
// 1) Tambah topik
async function addTopik(topik: any, token?: string): Promise<ApiResult<any>> {
  const res = await fetch(`${API_URL}/topik/addTopik`, {
    method: "POST",
...
// 2) Mapping materi ke topik
async function mappingMateri(param: any, token?: string): Promise<ApiResult<any>> {
  const res = await fetch(`${API_URL}/topik/mappingMateri`, {
    method: "POST",
    headers: getHeaders(token),
...
// 3) Edit topik
async function editTopik(param: any, token?: string): Promise<ApiResult<any>> {
  const res = await fetch(`${API_URL}/topik/editTopik`, {
    method: "PUT",
    headers: getHeaders(token),
    body: JSON.stringify(param),
  });
...
// 4) Edit mapping materi
async function editMappingMateri(param: any, token?: string): Promise<ApiResult<any>> {
  const res = await fetch(`${API_URL}/topik/editMappingMateri`, {
    method: "PUT",
    headers: getHeaders(token),
    body: JSON.stringify(param),
  });
...
// 5) Get detail topik
async function getDetailData(idTopik: string, token?: string): Promise<ApiResult<any>> {
  const res = await fetch(`${API_URL}/topik/getDetailData?id_topik=${encodeURIComponent(idTopik)}`, {
    method: "GET",
    headers: getHeaders(token),
  });
  const status = res.status;
...
const TopicDetailPages: React.FC = () => {
  ...
  // const apiUrl = import.meta.env.VITE_API_URL; // Sudah didefinisikan secara global di atas
  ...
  const loadDetailTopik = async (idTopik: string) => {
    try {
      ...
      const rawDataMateri = data.dataMateri || data.dataMateri || [];
      
      const tempMateri: Materi[] = rawDataMateri.map((m: any) => ({
        id: m.ms_id_materi || m.ms_id_materi,
        name: m.ms_nama_materi || m.ms_nama_materi,
